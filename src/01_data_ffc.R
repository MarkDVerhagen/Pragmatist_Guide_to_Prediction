...
##
## Description: Code replication from Salganik et al.
## to generate results for the FFC paper
##
## Data inputs:
## - data/ffc/background.dta
...
## -- Start code from Salganik et al.
...
d <- background %>%
  mutate(
    cm1relf = ifelse(cm1relf == 1, "Married",
  ...
  dplyr::select(
    challengeID, cm1ethrace, cm1relf, cm1edu,
...
benchmarks_long <- benchmarks %>%
  ...
  melt(
    id = c("challengeID", "outcome", "predictors"),
  ...
  mutate(
    r2_holdout = 1 - mean((truth - prediction)^2, na.rm = T) / mean((truth - ybar_train)^2, na.rm = T),
    beatingBaseline = r2_holdout > 0,
    outcome_name = case_when(
      outcome == "materialHardship" ~ "A. Material\nhardship",
      outcome == "gpa" ~ "B. GPA",
      outcome == "grit" ~ "C. Grit",
...
benchmarks_long %>%
  ...
  saveRDS("data/ffc/gpa_predictions_baseline.rds")

gpa_benchmark <- benchmarks_long %>%
  filter(outcome == "gpa") %>%
  saveRDS("data/ffc/gpa_predictions_baseline_full.rds")
...
estimates_with_intervals <- foreach(outcome_case = outcomes, .combine = "rbind") %do%
  {
    ...
    squared_errors <- squared_errors[, c(
      "baseline",
  ...
  mutate(
    outcome_name = case_when(
      outcome == "materialHardship" ~ "Material\nhardship",
      outcome == "gpa" ~ "GPA",
      outcome == "grit" ~ "Grit",
    ...
    outcome_name = fct_relevel(
      outcome_name, "Material\nhardship", "GPA", "Grit",
      "Eviction", "Job\ntraining", "Layoff"
    )
  )
...
saveRDS(sub_df, "data/ffc/all_submissions.rds")