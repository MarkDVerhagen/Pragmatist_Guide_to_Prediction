...
##
## Description: Script to generate Figure 2 in paper
##
## Data inputs:
## - data/ffc/main_plot.csv
## - data/ffc/gpa_predictions_baseline_full.rds
...
fig_1a <- ggplot(ffc_df, aes(y = bench, x = outcomes)) +
       geom_bar(
              stat = "identity", fill = MetBrewer::met.brewer("Egypt")[2],
              color = "black",
              alpha = 0.8
       ) +
       geom_text(
              label = fix_label(paste0(round(ffc_df$bench, 3) * 100, "%")), vjust = -1,
              size = 5, family = font_family
       ) +
       geom_rect(aes(
              ymin = rep(0.41, 6), ymax = rep(0.51, 6),
       ...
       scale_x_discrete(labels = c(
              "Material hardship", "GPA", "Job training",
...
ffc_gpa_full <- readRDS("./data/ffc/gpa_predictions_baseline_full.rds")
...
## Save for Figure 2.B
saveRDS(results_melt, "./data/ffc/gpa_predictions_null_baseline.rds")
...
fig_1b <- ggplot(results_melt, aes(y = value, x = test, fill = variable)) +
       geom_jitter(
              colour = "#333333",
              pch = 21,
              size = 3, alpha = 0.8, width = 0.05
       ) + # alpha = 0.2, size = 3, width = 0.04) +
       geom_hline(aes(yintercept = nul), linetype = "dashed") +
       cowplot::theme_cowplot() +
       theme_custom +
       facet_grid(rows = vars(variable)) +
       xlim(1, 4) +
       ylim(1, 4) +
       geom_abline(alpha = 0.7, linetype = "dotted") +
       theme_custom +
       xlab("Observed GPA") +
       ylab("Predicted GPA") +
...
load("./data/mortgage/mortgage_temp.rda")
...
results_melt$var_name <- factor(results_melt$var_name, levels = c(
       "m0", "m1", "m2",
       "m3", "m4", "m5"
))
...
rel_df <- results_group %>%
       filter(grepl("All|White|Black", sub)) %>%
       filter(!grepl("m2|m3|m5", var_name))
...
fig_1c <- ggplot(rel_df %>% filter(sub %in% c("White", "Black"))) +
       ...
       geom_point(
              data = rel_df %>% filter(!(sub %in% c("White", "Black"))), aes(y = mean, x = fct_rev(as.factor(var_name)), shape = "Overall"),
              size = 4, width = 2, stroke = 1
       ) +
       geom_text(
              data = rel_df %>% filter(!(sub %in% c("White", "Black"))),
...
desc_df_full <- rbind(desc_df, desc_df[desc_df$variable %in% c("mop_g", "lme_g"), ] %>%
       ...
       mutate(
              ptile_95 = ifelse(model == "Interval", NA, ptile_95),
...
plot_1d_df <- desc_df_full %>% filter(
       !grepl("Full", type),
       type == "No random effects",
       !grepl("Sex", spec)
)
...
fig_1d <- ggplot(plot_1d_df) +
       ...
       cowplot::theme_cowplot() +
       scale_fill_manual(
              name = "", values = MetBrewer::met.brewer("Lakota")[c(3, 6)],
...
fig_1e_df <- melt(resid, id.vars = "year") %>% mutate(
       variable = gsub("explained", "Explained", variable),
